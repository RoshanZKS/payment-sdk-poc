...
var _templateObject; // Use native Fetch API - no dependencies needed!
...
const apiClient = {
  async post(url, data) {
    ...
    const timeoutId = setTimeout(() => controller.abort(), _config.API_TIMEOUT);
    try {
      const response = await fetch("".concat(_config.API_BASE_URL).concat(url), {
        method: 'POST',
        headers: {
      ...
      if (!response.ok) {
        ...
        throw new Error(error.message || HTTP(_templateObject || (_templateObject = _taggedTemplateLiteral(["", ": ", ""])), response.status, response.statusText));
      }
      return {
